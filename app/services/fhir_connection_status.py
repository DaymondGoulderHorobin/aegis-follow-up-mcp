...
def get_fhir_connection_status(
    ...
    """Report FHIR-context header presence without performing live FHIR reads."""
    ...
    live_fhir_reads_enabled = (
        settings.live_fhir_reads_enabled
        and not settings.fixture_mode
        and not context.fixture_mode
    )
    active_data_source = (
        "external_fhir_server"
        if live_fhir_reads_enabled
        else "synthetic_fixture_data"
    )
    payload = {
...
def _notes(fhir_context_received: bool, active_data_source: str) -> list[str]:
