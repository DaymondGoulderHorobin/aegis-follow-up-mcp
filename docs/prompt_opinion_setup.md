...
5. Confirm tool discovery lists the final workflow, FHIR transparency, optional FHIR proof, handoff, and AI narrative tools.
...
17. Optionally invoke `validate_fhir_context_connection` only when FHIR context is supplied and live reads are explicitly enabled.
18. Invoke `create_follow_up_handoff_payload` for `synthetic-patient-003`.
19. Invoke `update_follow_up_task_status` for a demo task.
20. Invoke `get_ehr_integration_summary`.
21. Record any initialization, trust-toggle, header-forwarding, or timeout notes for demo rehearsal.
...
All scopes are optional by default. Users can leave the extension disabled and still use the deterministic synthetic demo. The server does not request `offline_access`, does not receive refresh tokens, and does not use live FHIR for clinical logic. The optional `validate_fhir_context_connection` proof performs only a read-only Patient reachability check when context is supplied and `LIVE_FHIR_READS_ENABLED=true`.
...
Use `validate_fhir_context_connection` only as an optional feasibility check. It
returns reachability metadata and never returns access tokens, patient
demographics, or the full FHIR response body.
...
```text
If FHIR context is supplied and live reads are enabled, validate the FHIR context connection without exposing tokens or PHI.
```
...
- Missing tools: open `/version` and verify the deployed code is version `0.10.0`.
